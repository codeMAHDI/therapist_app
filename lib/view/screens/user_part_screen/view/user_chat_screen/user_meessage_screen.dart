...
class UserMeessageScreen extends StatefulWidget {
  const UserMeessageScreen({super.key});

  @override
  State<UserMeessageScreen> createState() => _UserMeessageScreenState();
}

class _UserMeessageScreenState extends State<UserMeessageScreen> {
      ...
      switch (controller.messageStatusLoading.value) {
        ...
        case Status.internetError:
          return GeneralErrorScreen(
              onTap: () => controller.getAllMessage(conversationId: chat.id ?? ''));
        case Status.error:
          return GeneralErrorScreen(
              onTap: () => controller.getAllMessage(conversationId: chat.id ?? ''));
            ...
            appBar: AppBar(
              ...
              title: Row(
                ...
                children: [
                  Row(
                    children: [
                      Stack(
                        children: [
                          CustomNetworkImage(
                            imageUrl: ImageHandler.imagesHandle(controller.therapistImageByAppointmentId[chat.appointment ?? ''] ??controller.therapistImageByUserId[chat.therapist?.therapistUserId?.id ?? ''] ??''),
                            height: 45.h,
                            width: 45.w,
                  ...
                  //         Get.to(() => CallingPage(
                  //               callID: chat.id ?? '',
                  //               userName: chat.patient?.name ?? '',
                  //               userID: chat.patient?.patientUserId?.id ?? '',
                  //             ));
                  //       },
                  //     ),
                  //     IconButton(
              ...
              children: [
                ...
                Expanded(
                  child: Padding(
                    ...
                    child: Obx(() {
                      final avatarUrl = ImageHandler.imagesHandle(controller.therapistImageByAppointmentId[chat.appointment ?? ''] ??controller.therapistImageByUserId[chat.therapist?.therapistUserId?.id ?? ''] ??'');
                            ...
                            child: CustomTextField(
                              ...
                              suffixIcon: IconButton(
                                icon: const Icon(Icons.send,
                                    color: AppColors.primary),
                                onPressed: () {
                                  if (controller.messageController.value.text.isNotEmpty ||controller.selectedImages.isNotEmpty) {
                                    controller.sendMessage(conversationId: chat.id ?? '',senderId:chat.patient?.patientUserId?.id ?? '',
...
class CustomInboxMessage extends StatelessWidget {
  ...
  final String avatarUrl; // New parameter to fetch the avatar url of the therapist
                        ...
                        child: GestureDetector(
                          onTap: () {
                            Get.to(() => ShowImageScreen(
                              imageUrl:  ImageHandler.imagesHandle(imageUrls[index]),
